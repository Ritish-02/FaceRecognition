...
class Student:
    def __init__(self,root):
        ...
        #first image
        img = Image.open(r"D:\FaceRecognition\college_images\student_attendance.jpeg")
        ...
        #second image
        img1 = Image.open(r"D:\FaceRecognition\college_images\std.jpeg")
        ...
        #third image
        img2 = Image.open(r"D:\FaceRecognition\college_images\student.jpeg")
        ...
        #bg image
        img3 = Image.open(r"D:\FaceRecognition\college_images\bg_image.jpg")
        ...
        img_left = Image.open(r"D:\FaceRecognition\college_images\s.jpeg")
        ...
        dep_combo = ttk.Combobox(current_course_frame,textvariable=self.var_dept,font=("times new roman",12,"bold"),state="readonly",width=20)
        dep_combo['values']=("Select Department","Computer Science","IT","ECE","Mechanical")
        dep_combo.current(0)
        ...
        course_combo["values"] = ("Select Course","B.Tech","B.E","M.Tech","M.B.A","B.C.A")
        ...
        img_right = Image.open(r"D:\FaceRecognition\college_images\s1.jpeg")
    ...
    def add_data(self):
        if self.var_dept.get() == "Select Department" or self.var_std_name.get() == "" or self.var_std_id.get() == "":
            ...
            try:
                ...
                self.fetch_data()
                conn.close()
                messagebox.showinfo("Sucess","Student Details has been addded successfully",parent=self.root)
    ...
    def generate_dataset(self):
        ...
        try:
            ...
            # Update student data before collecting dataset
            my_cursor.execute("""
                UPDATE student 
                SET Dep=%s, Course=%s, Year=%s, Semester=%s, Name=%s, Division=%s, 
                    Roll=%s, Gender=%s, DOB=%s, Phone=%s, Address=%s, Teacher=%s, 
                    PhotoSample=%s, Email=%s 
                WHERE Student_Id=%s
            """, (
                self.var_dept.get(),
                self.var_course.get(),
                ...
                self.var_address.get(),
                self.var_teacher.get(),
                "Yes",  # Setting 'PhotoSample' to Yes
                self.var_std_id.get()
            ))
            ...
            img_id = 0
            student_id = self.var_std_id.get()  # Ensure correct ID usage
            ...
            while True:
                ...
                if cv2.waitKey(1) == 13 or img_id == 100:  # Stop after 100 samples or when Enter key is pressed
                    break
